 ...
 *  - Video upload no longer sets Content-Type: multipart/form-data manually;
 *    letting FormHttpMessageConverter set it lets Spring add the required
 *    multipart boundary parameter, fixing a 422 Unprocessable Entity from
 *    the AI service (see uploadVideo() for details).
 */
@RestController
...
public class MonitoringController {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> uploadVideo(
        ...
        try {
            ...
            // NOTE: Do NOT set Content-Type to MULTIPART_FORM_DATA here.
            // FormHttpMessageConverter only generates the required
            // "boundary=..." parameter when the outgoing message has NO
            // Content-Type set yet. If we set it ourselves (even to the
            // correct-looking "multipart/form-data"), the converter sees a
            // Content-Type already present and leaves it untouched — so the
            // request goes out as "multipart/form-data" with NO boundary.
            // FastAPI/Starlette cannot split the body into parts without a
            // boundary, so it sees "video" and "camera_id" as missing and
            // returns 422 Unprocessable Entity. This is the root cause of
            // the video upload failure; leave headers empty and let Spring
            // add the correct Content-Type (with boundary) automatically.
            ...
            String targetUrl = aiServiceUrl + "/api/analyze/video";
            log.info("Uploading video to AI service: url={}, filename={}, size={} bytes, camera_id={}, capacity={}",
                    targetUrl, videoFile.getOriginalFilename(), videoFile.getSize(), cameraId,
                    camera.getMaximumCapacity());
            log.debug("Video upload multipart fields: {}", body.keySet());

            ResponseEntity<Map> aiResponse = restTemplate.postForEntity(targetUrl, requestEntity, Map.class);

            return ResponseEntity.ok(ApiResponse.success("Video analysis started", aiResponse.getBody()));

        } catch (HttpStatusCodeException e) {
            // AI service responded, but rejected the request (e.g. 422 from
            // FastAPI/Pydantic validation). Log everything needed to debug
            // it: status, full response body (FastAPI validation errors are
            // JSON describing exactly which field/type failed), and the
            // request headers we sent.
            log.error(
                    "AI service REJECTED video upload for camera {} (HTTP {}): responseBody={}, requestHeaders={}",
                    cameraId, e.getStatusCode(), e.getResponseBodyAsString(), e.getResponseHeaders());
            return ResponseEntity.ok(ApiResponse.error(
                    "AI service rejected the video upload (HTTP " + e.getStatusCode() + "): "
                            + e.getResponseBodyAsString()));
        } catch (ResourceAccessException e) {
            log.warn("AI service unreachable for video upload: {}", e.getMessage(), e);
            return ResponseEntity.ok(ApiResponse.error(
                    "AI service is not running. Start it with: python main.py"));
        } catch (Exception e) {
            log.error("Video upload error for camera {}: {}", cameraId, e.getMessage(), e);
